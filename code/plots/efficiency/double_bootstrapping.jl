...
for i in range(1, mult_depth)
    res.data = OpenFHE.EvalBootstrap(SecureArithmetic.get_crypto_context(context), res.data, 2, 19)
...
for i in range(1, length(mult_depth_range))
    ...
    for i in 1:6
        start = time_ns()
        OpenFHE.EvalBootstrap(SecureArithmetic.get_crypto_context(context), ciph_sin.data, 2, 19)
        finish = time_ns()
...
CSV.write("out/efficiency/double_bootstrapping.csv", table)
