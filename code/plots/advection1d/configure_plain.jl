...
advection1d("PLAINTEXT", "Lax-Wendroff", N, N_steps, timespan, a, mult_depth, true)
# second run
timer_conf_LW_vec = []
for i in range(1, 100)
    local timer_conf, _, _ = advection1d("PLAINTEXT", "Lax-Wendroff", N, N_steps, timespan,
                                         a, mult_depth, true)
...
GC.enable(false)
# first run is not representative in sense of time
advection1d("PLAINTEXT", "1st order FD", N, N_steps, timespan, a, mult_depth, true)
# second run
timer_conf_FD_vec = []
for i in range(1, 100)
    local timer_conf, _, _ = advection1d("PLAINTEXT", "1st order FD", N, N_steps, timespan,
                                         a, mult_depth, true)
...
CSV.write("out/advection1d/conf_time_plain.csv", table_time)
