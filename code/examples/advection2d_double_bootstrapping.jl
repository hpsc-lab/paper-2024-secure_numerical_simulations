...
function generate_setup(a, timespan, N_steps, span, N, context::SecureContext,
                        depth, init_cond::Function, public_key)
    ...
    ode = ODE(u0, eq, semi, timespan, N_steps, depth)
    return ode
end
...
function solution_linear_advection_equation2D(a, timespan, span, N, init_cond::Function)
    ...
    for i in range(1, N)
        for j in range(1, N)
            sol[i, j] = init_cond([span[1] + dx*(i-1) - timespan[2]*a[1],
...
function advection2d(mode, method, N::Int, N_steps::Int, timespan, a, mult_depth::Int,
                     secure::Bool=false, callback=nothing)
    ...
    if mode == "CIPHERTEXT"
        ...
        if secure
            ...
            cc, mult_depth =  generate_cryptocontext(1<<13, mult_depth, N_round)
        end
        ...
        u = solve_double_bootstrapping(ode, method, callback)
